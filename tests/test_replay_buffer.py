...
def test_sample_output_shapes(batch_size):
    buf = ReplayBuffer()
    _fill_buffer(buf, batch_size + 10)
...
def test_single_transition_round_trip():
    """Push one transition, sample it back, verify all fields match."""
    buf = ReplayBuffer()
...
def test_sample_has_no_duplicates():
    """replace=False should mean no duplicate transitions in a batch."""
    buf = ReplayBuffer()
    n = 50
    for i in range(n):
        t = _make_transition(seed=i)
        t["reward"] = float(i)  # unique stamp
        buf.push(t)
